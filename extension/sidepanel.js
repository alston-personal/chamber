...
async function showReborn() {
  ...
  let identity = null;
  try {
    identity = await activePlatformIdentity();
    platform = identity?.platform || "facebook";
  } catch (_) {
    try {
      const tab = await getActiveTab();
      platform = (tab && platformForTab(tab)) || "facebook";
    } catch (_) {
      platform = "facebook";
    }
  }
  ...
  backupView.hidden = true;
  settingsView.hidden = true;
  const proV = document.getElementById("proView");
  ...
  if (!rebornText.value.trim() || rebornText.dataset.autoGenerated === "true") {
    const key = platform === "threads" ? "declaration.defaultTextThreads" : platform === "x" ? "declaration.defaultTextX" : "declaration.defaultText";
    rebornText.value = t(key, { platform: name, timelineUrl });
    rebornText.dataset.autoGenerated = "true";
  }
  rebornStatus.textContent = t("reborn.identity", { alias });
}
...
rebornGenerate?.addEventListener("click", async () => {
  ...
  try {
    const tab = await getActiveTab();
    const platform = tab ? platformForTab(tab) : null;
    const state = await getActiveProfile();
    const profile = state?.profiles?.find((item) => item.id === state.activeId);
    const alias = profile?.alias || "chamber";
    const timelinePlatform = platform === "threads" ? "threads" : platform === "instagram" ? "instagram" : platform === "x" ? "x" : "facebook";
    ...
    if (platform === "threads") {
      ...
      rebornStatus.innerHTML = (result.imageAttached ? t("reborn.successPlatform", { platform: "Threads" }) : t("reborn.successTextOnly", { platform: "Threads" })) +
        `<br><span style="font-size:11px;opacity:0.9;">💡 文案與圖片已同步複製至剪貼簿，若有缺漏亦可按 Ctrl+V 貼上。</span>`;
