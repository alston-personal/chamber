/**
 * Pure Offline QR Code Generator for Chamber Extension
 * Zero Network Dependency, 100% Local Canvas Rendering
 * Based on QR Code Model 2 algorithm (Kazuhiko Arase / MIT)
 */
(function (global) {
  //---------------------------------------------------------------------
  ...
  const QRUtil = {
    PATTERN_POSITION_TABLE: [
      [], [6, 18], [6, 22], [6, 26], [6, 30], [6, 34], [6, 22, 38], [6, 24, 42], [6, 26, 46], [6, 28, 50], [6, 30, 54], [6, 32, 58], [6, 34, 62], [6, 26, 46, 66], [6, 28, 48, 70]
    ],
  ...
  /**
   * Render QR Code to HTML Canvas element (100% pure client side, zero network requests)
   */
  function renderQRCodeCanvas(text, options = {}) {
    const size = options.size || 250;
    const margin = options.margin != null ? options.margin : 2;
    const colorDark = options.colorDark || "#6366f1";
    const colorLight = options.colorLight || "#020617";
  ...
  global.ChamberQRCode = {
    render: renderQRCodeCanvas,
    drawToContext: function (ctx, text, x, y, size, colorDark = "#6366f1", colorLight = "#020617") {
      const qrCanvas = renderQRCodeCanvas(text, { size, colorDark, colorLight, margin: 1 });
