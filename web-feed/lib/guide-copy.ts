...
const zhTW: GuideCopy = {
  ...
  mappingTitle: "完成 Facebook mapping", mappingIntro: "Mapping 把目前登入的 Facebook 帳號、Chamber 暱稱和備份使用的儲存帳號登記成同一身分。Chamber 用它判斷文章是不是本人發布，並決定備份出現在哪個 Echo 迴音谷；它不會取得 Facebook 密碼，也不需要 Facebook token。",
  mappingSteps: [
  ...
  mappingImportant: "重要：mapping 只負責身分與迴音谷歸屬；真正解密的是本機金鑰。完成 mapping 後，請在 Echo 建立 2-of-3 復原設定：A 留在 Extension、Passkey 保護 Vault B、C 由你離線保存。C 只在建立期間顯示。",
  backupTitle: "備份一篇 Facebook 文章", backupSteps: ["登入 Facebook，進入已完成 mapping 的本人個人檔案頁。", "開啟 Chamber 側邊欄，確認 Chamber 與 Facebook 帳號正確。", "按「在 Facebook 選取文章」，再點文章本文、圖片或空白處；按 Esc 可退出。", "核對文字、圖片、時間與原文連結；依提示展開文字或完整載入相簿。", "按「備份這篇」。成功後可開啟單篇 Echo 或完整迴音谷。"],
  ...
  echoTitle: "在 Echo 閱讀備份", echoBody: "使用相符 Chamber 身分開啟 Echo 後，迴音谷會自動解鎖，不必逐篇操作。相簿可左右瀏覽；若自動解鎖未完成，可重新解鎖。",
